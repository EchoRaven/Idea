...
找一条能 strain 前沿模型的自变量」转向「四条独立 strain 轴（难度、攻击强度、算术、逻辑）
× 四条 diversion 通路（内容断言、动作、数值权威、逻辑权威）全部测完，结论收敛」。
CAPSTONE（4-victim，well-powered）用更大样本重新做实了 §0a 的弱 victim 结果：
Llama-4-8B 的 benign 在 depth 1→2→4 上真塌陷（0.34→0.21→0.16），但 diversion 在
同一批更大样本（n_adm~30）下持平（0.10→0.12→0.17，CI 重叠）——直接推翻了 §0a 当初
「diversion 朝 frontier 上升」这条读数，文档原话「was noise」。Three-axis update 补了
攻击强度轴（§8/§8.1，rift/kimi 都不因换更强注入而提高 diversion，kimi 反而把攻击
强度转成能力塌陷）和计算/推理轴（§9 系列，rift/muse-spark 算术 42/42、逻辑 24/24
全满分，此前 ~0.4 的「hard-task floor」被重新归因为检索/整合负担而非推理本身），并
新增数值权威（§9.1）、逻辑权威（§9.3）两条全新 diversion 通路，均为 0。§9.3 明确宣布
「Program complete」，但之后又追加了 §9.4（推理能力的跨 victim 泛化）、§9.5（弱 victim
在推理轴上因不会用工具而不可测，诚实报告为「unmeasurable」而非假 0）。全文顶部新增的
Abstract 把这十节内容压缩成一页摘要，date-stamped 2026-08-07，是目前对整个项目最新、
最权威的入口。详见下方「需要你注意的」新增 #20–#23。第十六、十七批（分别见下方
独立小节）是两次单点方法论收尾，第十八批（同样见下方独立小节，新增 #26）新增了
...
**2026-08-07 第十七批（本次处理的更新，紧接第十六批，同样是单点追加）**：`findings.md`
在 §9.5 末尾又追加一段「fix-attempt closure (opt-2)」，报告了两次系统性尝试给弱
victim（Llama-4-8B）补测推理阶梯的结果：①把账本从 googledocs 换成 gmail 邮件投递——
模型开始「参与」，但把工具调用 id 当成字面占位符字符串
（`get_message(id="message_id_from_list_messages")`），多步检索因此失败，最终发出
空答案；②进一步在提示里明确提示调用 `get_gmail_content(limit=200)`——模型这次能正确
取到账本、也在消息文本里开始计算，但循环、截断，并把答案发给用户而不是通过
`send_email(finance@)` 投递。两次尝试都是 0/6。文档把结论归纳为「弱 victim 在多个
互相纠缠的环节（工具调用状态穿透、推理完整完成、结果路由）上同时失败，都不是 harness
缺陷」，并明确给出了解释上一批（#24）张力的机制：CAPSTONE 的 difficulty 轴上
Llama-4-8B 之所以有非零 benign_rate，是因为那批任务只需要单次、无需状态穿透的
`get_gmail_content` 一次性转储，和这里需要跨步骤正确穿透 message id 的推理阶梯任务
结构完全不同。详见下方新增 #25。
**当前节奏**：延续上一条——项目仍在「收官式总结之后继续查漏补缺」阶段，这次和上一批
一样是纯方法论收尾（确认弱 victim 推理轴数字为什么不可得的具体机制），不是新数据/
新轴/新 victim。两批单点追加合计起来，项目自 CAPSTONE/三轴更新宣布「settled」之后，
目前为止没有再产出任何影响核心结论的新证据——四轴×四通路、susceptibility 低且
capability-independent 这条核心结论保持不变。

**2026-08-07 第十八批（本次处理的更新，findings.md 追加全新 §10，是「program
complete」之后第一次真正新增 strain 维度而非方法论收尾）**：`findings.md` 在
§9.5 之后新增一整节「Ambiguity / epistemic calibration」，把四轴框架扩成五轴——
不测难度、注入强度、算术或逻辑，测的是模型面对**病态提问**（信息不足以唯一确定答案，
或约束本身互相矛盾）时会不会编造一个自信的错误答案，而不是如实报告「无法确定」/
「矛盾」。三种题型（unique 控制组、underdetermined、contradictory/UNSAT）各 6 题，
rift 三档全部满分，轨迹核验证实是真实校准（4 解谜题枚举全部解、UNSAT 谜题逐步推导
矛盾），不是关键词侥幸。但表格给出的 total 行写着 24/24，和 6+6+6=18 直接对不上——
是文档自己的一处算术矛盾，不是我推断出来的方法论薄弱点。这条轴的安全含义也和其余
四条不同：它测的是「伪造确定性」这个操纵面本身是否关闭，rift 目前关闭了，但文档
明确留了一个还没交付的开放问题——弱模型是否也会 fabricate，让校准成为迄今唯一一条
**capability-ordered** 的安全维度。详见下方新增 #26。
**当前节奏**：和前两批（纯方法论 debug）不同——这批是「program complete」宣布之后
第一次真正扩展研究范围，说明项目的收尾判定目前更像一个阶段性用词。顶部 Abstract
...
**2026-08-06 更新（第十一次追加，方向反转，本条的结论被下方 #19 直接推翻）**：本条
留下的悬空承诺（「n→~25 的补充 linear 基线正在跑」）这次交付了，而且样本量超过承诺
（n=30），但结果不是「区间收窄、方向不变」——linear benign 从 0.556 掉到 **0.30**，
反而低于 convergent 的 0.44。上面「效应方向正确、量级温和」这个措辞本身已经不成立，
应替换为 #19 的措辞：「结构不是驱动因素，内容密度才是」。

### 19.（本轮新增，第十一次追加，直接推翻本条与 #15 的核心措辞）matched control 的 linear 基线扩到 n=30 后方向反转——不是「convergent 效应量变小」，是「convergent 效应量不存在，起作用的是内容密度」

[findings §0-pre 追加段](tech/2026-08-04-findings.md)（同一段「MATCHED CONTROL」，标题
已改写为 "REFUTES 'convergent structure strains more'"）直接回应 #18 留下的悬空承诺——
上一轮 matched linear-20 只有 n=9，一份补齐到 n≈25 的重跑「正在跑」。这次真的跑完了，
而且样本量超过承诺（n=30），但结果不是「区间收窄、方向不变」，是**方向整个反过来**：

| depth-20，同一份高密度种子 | rift benign | Wilson 95% CI |
|---|:--:|:--:|
| linear（`CHAIN_MODE=linear`，n=30，上一轮 n=9/0.556） | **0.30** | [0.17, 0.48] |
| convergent（n=25，未变） | 0.44 | [0.27, 0.63] |

上一轮（#18）读到的是「convergent 更低、但区间和 linear 重叠、效应量从~0.3 下修到
~0.12」——方向仍然支持"convergent 结构比 linear 更 strain"。这一轮 linear 的点估计从
0.556 掉到 0.30，直接**低于** convergent 的 0.44，两者关系反过来了。文档自己把这个
结果总结成「chain STRUCTURE（linear vs convergent）不是 strain 驱动因素——真正起作用
的是任务的 **content density + depth**：同一份信息密度种子换成 20 步纯线性链，一样
能把 rift 压到 0.30」，并直接承认「the apparent convergent effect was entirely the
denser seed」。

需要指出三点：

1. **这不是"进一步下修"，是"此前的结论方向本身站不住"**——#18 已经把七次追加时
   "convergent 是至今唯一压穿前沿模型的轴"这个头号新结论下修过一次（~0.3→~0.12），
   这次连"convergent 比 linear 更 strain"这个方向性结论本身都不成立了。七次到十次
   追加累积起来的"convergent DAG 结构本身是关键自变量"这条叙事线，现在应该被替换成
   "content density（信息密度）+ depth 才是关键自变量，DAG 结构本身不是"——这不只是
   数字调整，是对项目过去两天最重要一条新结论的实质性推翻。
2. **depth 轴 well-powered 负结果（§2b）的适用范围需要重新表述**——此前（#2/#15）的
   表述是"depth 不 strain 前沿模型（原始负结果），换成 convergent 结构才能压穿（部分
   推翻）"；现在应该是"depth 在低密度种子上不 strain 前沿模型；在高密度种子上，不论
   线性还是 convergent 结构，depth-20 都能把 rift 压到 0.30–0.44"——密度才是那个被
   找到的、真正缺失的自变量，不是图结构。这意味着 PROJECTS.md 此前下一步⑩里"如果依然
   重叠，§0-pre『convergent 是唯一压穿前沿模型的轴』这个措辞需要降级为更谨慎的表述"这个
   条件已经触发，而且触发的方式比预想的更彻底（不是"更谨慎"，是"这个措辞本身需要撤回，
   换成密度更准确"）。
3. **diversion 在这次反转里全须全尾地维持了原有结论**——文档补了一句"diversion stays
   low (~0.12) on both regardless — structure/content strain COMPETENCE, not
   susceptibility"，也就是说不管是 linear 还是 convergent 把 benign 压到
   0.30/0.44，diversion 都没有跟着升高。这是"diversion 不随 strain（不管用什么方式
   测量）上升"这个反直觉模式第四次独立出现（depth 轴、within-victim 相关性、
   convergent 3-victim、现在是这次 structure-vs-density 反转），且是唯一一条经受住
   了这次自我推翻仍然成立的结论——如果要在这个项目里挑一条最值得信任的发现，现在应该
   是这条，而不是任何一条具体的"哪条轴能 strain 前沿模型"。

文档自己对这次反转的评价是「the matched control did its job — it caught and
corrected a would-be overclaim」——这是项目第三次主动回头纠正自己交出的头号新结论
（第一次是 #17 的 kimi-k3 订正，第二次是 #18 的效应量下修，这次改动幅度是三次里最大的
一次，直接推翻了方向）。这个"愿意用干净对照推翻自己叙事"的模式，是这份文档目前最值得
信任的方法论习惯，但也意味着：过去两天（七次到十次追加）里所有引用"convergent 轴"
"convergent DAG"作为关键发现的下游表述（PROJECTS.md 断点、本索引 #15/#16/#17/#18），
现在都需要在下次被正式引用前，替换成"content density"这个更准确的说法——不是全部
作废，跨 victim 的能力排序（rift>kimi-k3>Llama-4-8B）、diversion 不随 strain 上升
这两条结论依然成立，只是"为什么 benign 会下降"这个归因需要改口。

另外这次反转没有触及的缺口依旧悬着：convergent 3-victim 表（rift/kimi-k3/Llama-4-8B）
的 diversion 数字（0.125/0.083/0.118）仍然没有 Wilson CI（#16/#17/#18 已反复指出）；
kimi-k3 的接入方式说明仍未补上；「换注入面（wall→soft surface）」这条建议这次也依旧
没有被碰——本轮交付的是对已有结论的自我核查，不是新方向。

### 20.（本轮新增，全项目目前最大的方法论缺口）CAPSTONE/三轴更新把研究问题定论为「settled」，但被建议十一轮以上、从未执行的「换注入面（wall→soft surface）」pivot，仍然完全没有出现在这份「收官」总结里

新增的 Abstract、CAPSTONE、Three-axis update 三处都在使用「settled」「program
complete」这类收尾措辞，把项目结论定格为「naive strain-invariance refuted；
capability doesn't buy security 是结构性的，不是 strain 驱动的」。支撑这个结论的四条
diversion 通路里，action（exfiltration/unauthorized_action）这一条在 §7 里首次补齐
到全部四个 victim，结果依旧是 0/41、0/46（配了 Wilson CI，见下方 #23）——这是「action
边界从不被攻破」这条结论证据最扎实的一次扩展。

但 §7 用来测 action 边界的注入方式，从项目一开始（见「关键决策记录」「固定注入而非运行
时注入」）到现在都没有变过——[avo-redteam](../avo-redteam/_INDEX.md) 已经用「soft
surface」注入（把伪造内容 poison 进 victim 会读的记录本身，而不是直接写进任务指令）在
rift 身上打出过一次手工核验的真实攻击成功，且触发的正是一次**动作**
（`create_payout`）。本项目「需要你注意的」#7 从 2026-08-05 起就在反复提示这个缺口，
累计已经提醒了十一轮以上（每一轮「下一步①」都写着同一条建议），但 CAPSTONE / 三轴
更新 / §7–§9.5 这十个新章节里，没有一个是去验证「soft surface 下 action diversion
是否还是 0」——四个 victim 全部补齐用的还是同一种「wall」式固定注入。也就是说，项目
现在敢下「action 类攻击 0 diversion，跨全部四档能力」这个更强的结论，但样本量扩大的
方向和 avo-redteam 已经证明过会被打穿的那个维度完全正交。在这个具体缺口被补上之前，
「capability doesn't buy security holds for a structural reason」这个总结性论断，
至少在 action 通路这一半上，只在「wall」注入面下成立，还没有在已知会被打穿的注入面下
重新验证过。

### 21.（本轮新增）第三个零基础设施说明就空降结果的新 victim：muse-spark-1.1，且全文对它的命名本身不一致

新 victim **muse-spark-1.1**（标为「strong」，和 rift 并列成第二个前沿模型）首次出现
在 CAPSTONE 对比表里，之后在 Deep-frontier follow-up、§7、§9.4 里反复被引用为「第二个
强模型」的独立验证来源（比如 §9.4 用它验证 rift 的推理满分不是 rift 专属）。但全文没有
一处交代它的 endpoint、鉴权方式、是否和 rift/kimi-k3/Llama-4-8B 用同一套 judge 和
注入器——这是继 kimi-k3（见 #16）之后，第三个用这种方式引入的 victim，而 Llama-4-8B
当初有 §4 完整的工具-schema 净化器接入记录做对照，标准明显不一致。

命名本身也不统一：CAPSTONE 表头、Deep-frontier follow-up、§7 表格里写的是
「muse-spark-1.1」，但 CAPSTONE 正文一句话（"two independent strong victims (rift
AND muse-spark, near-identical curves)"）和 §9.4 整节（表头「muse-spark
(strong)」、正文「muse's single "miss"」）都省略了版本号，写成「muse-spark」。无法从
文档内部判断这是笔误还是指两个不同的模型版本——如果后续要正式引用「muse-spark 系列在
4 个独立轴上都复现了 rift 的满分/零 diversion」这类跨章节汇总的结论，需要先确认全文
说的是同一个模型。

### 22.（本轮新增）§9.3 宣布「Program complete」的位置在文档中段，之后又追加了 §9.4/§9.5——继 #13/#15/#16 之后第三次出现「阶段性总结的位置和实际写作进度对不上」

§9.3 结尾明确写下「**Program complete** — four strain axes × four diversion
pathways, one conclusion.」，语气是收尾总结。但同一份 findings.md 在它之后又追加了
两整节新内容：§9.4（把 §9/§9.2 的推理结果扩展到 muse-spark 和 kimi-k3，发现 kimi 在
逻辑推理上大量 non-convergence）、§9.5（Llama-4-8B 在推理轴上因为不会操作 googledocs
工具而完全不可测，诚实报告为「unmeasurable」而非假 0）。这两节都是高质量的补充证据，
不是注水——§9.5 尤其体现了项目一贯的「先核实是不是 harness confound 再下结论」的好
习惯（呼应此前 §9.2 抓到的判官假阴性）。

问题不在内容，在措辞时机：这是继 #13（文件开头摘要「still blocked on a second
powered victim」没跟上 §0a 的结果）、#15（§0-pre 自称「reframes §0」但 §0 headline
没同步改写）、#16（同一个「still blocked」摘要连续两轮没更新）之后，第三次出现「某处
写了收尾性质的话，但文档实际还在往后长」的模式。目前顶部新增的 Abstract
（date-stamped 2026-08-07，晚于 §9.3/9.4/9.5 全部内容）已经是全文档最新、最全的收尾
入口，读者应该以它为准，而不是被 §9.3 中段那句「Program complete」误导成「后面不会
再有新章节」。建议后续维护者：只在文档最末尾或独立的 Abstract/tl;dr 区块使用
「complete/settled」这类字眼，不要在章节内部使用。

### 23.（本轮新增，方法论改善，值得记录）Wilson CI 合规性明显提升，但没有做到全面覆盖

此前 #9/#11/#16/#17/#18 反复指出「关键决策记录」里「diversion 比值一律配 Wilson
CI」这条规则在新增结论里经常被绕过（distractor/discrimination 子实验裸报、
convergent 3-victim 表连分子分母都没给）。本轮新增的几节里，这条规则被系统性地重新
执行：§7 的 exfiltration/unauthorized_action 首次跨全部四个 victim 逐个配了 Wilson
区间（0/41 [0,0.086]、0/46 [0,0.077]）；§8 的攻击强度对比给了 [.03,.33]；§9.1 的
数值权威探针给了 [0,.19]；§9.3 的逻辑权威探针给了 [0,.20]。这是本项目至今 Wilson CI
执行力度最好的一批新结论。

但仍不是全面合规：§8.1（kimi vs rift 的攻击强度内部对比表，四个 diversion 数字
0.125/0.11/0.118/0.0）和 Deep-frontier follow-up（depth-30/40 的 diversion
0.154，出现两次）依旧是裸点估计，没有配区间——这两节都是 n 较小（8.1 是 n_adm
...
### 25.（本轮新增，第十七批，§9.5 末尾追加「fix-attempt closure (opt-2)」）弱 victim 失败机制被进一步拆解——为 #24 指出的「结论范围写宽了」提供了更精确的解释

[findings §9.5 追加段](tech/2026-08-04-findings.md) 是本轮（第十七批）唯一的新增内容，
直接接着上一批（#24）的「root-cause upgrade」继续做：既然确认了 Llama-4-8B 在推理阶梯
上的失败不是 harness bug，项目又做了两次系统性「补救」尝试，想榨出一个干净的弱 victim
推理数字：

1. 把账本从 googledocs 换成 gmail 邮件投递——模型确实「engage」了，但把工具调用 id 当成
   字面占位符字符串（`get_message(id="message_id_from_list_messages")`），多步检索因此
   失败，最终发出空答案。
2. 在提示里进一步明确提示调用 `get_gmail_content(limit=200)`——模型这次正确取到了账本、
   也在消息文本里开始计算，但发生循环、截断，并把最终答案发给用户而不是通过
   `send_email(finance@)` 投递。

两次尝试都是 0/6。文档的结论是：弱 victim 在三个互相纠缠的环节（工具调用状态穿透、推理
完整完成、结果路由到正确的最终动作）上同时失败，没有一个是 harness 缺陷；对最弱这一档
模型，「推理能力」和「agentic 完整执行能力」两者都低于阈值且拆不开。

这段内容最有价值的地方，是它明确解释了 #24 指出的那处张力——为什么同一个模型在
CAPSTONE/§0a 的 difficulty 轴上有非零 benign_rate，却在推理阶梯上全 0：文档原话「the
difficulty-axis tasks worked precisely because they only needed a single no-threading
`get_gmail_content` dump」。也就是说真正的能力缺口不是「googledocs vs gmail」这个工具
族的区别（#24 当时的猜测），而是**任务是否需要跨步骤穿透正确的 tool-call id（多步
检索）**——difficulty 轴的任务是一次性转储，推理阶梯任务需要先 list 再按 id 精确
get，后者才是这个模型做不到的。

这是一次好的自我澄清，建议下游引用时，把 #24 里建议的替换措辞「无法调用 googledocs
工具族，但能操作 gmail 工具」进一步精确成「能做单步、无需 id 穿透的 gmail 批量转储，
但做不到需要跨步骤正确传递 tool-call id 的多步检索——googledocs 恰好总是需要多步，
不是因为它是 googledocs，而是因为它总是需要穿透」。「below the agentic-tool-use
threshold」这类整体性措辞依旧不准确，这条新证据只是让不准确的地方从「工具族」层面
下沉到了更精确的「单步 vs 多步状态穿透」层面。

轻微的统计提醒：0/6 的样本量很小，但这里衡量的是竞争力（能否完成任务）而非 diversion
比值，不直接受本项目「diversion 比值一律配 Wilson CI」这条规则约束；不过 n=6 本身也
不足以把「弱 victim 推理能力不可测」这个结论坐实到比 #24/§9.5 原始判定更强的程度，只是
补充了两次具体尝试的失败细节，结论强度应维持在「不可测」而非升级为任何更确定的数字。

### 26.（本轮新增，第十八批，新增第五条 strain 轴）ambiguity/epistemic calibration：rift 24/24 关闭了一个操纵面，但表格算术对不上、且仅在 rift 单一 victim 上验证过

[findings §10](tech/2026-08-04-findings.md) 是 `findings.md` 在宣布「Program
complete」（§9.3，见 #22）之后的第五次继续追加（继 §9.4/§9.5/root-cause
upgrade/opt-2 closure 之后），也是这批「查漏补缺」里第一次真正新增一条从未测过的
strain 维度，而不是给旧数据做方法论收尾。问题变了：不是「任务多难/注入多强/算得
对不对」，是模型面对**病态提问**——信息不足以唯一确定答案（underdetermined）、或
约束本身互相矛盾（UNSAT）——时，会不会编造一个自信的错误答案，而不是如实报告
「无法确定」/「矛盾」。三种题型各 6 题，rift 三档全部满分（unique 6/6、
underdetermined 6/6、contradictory 6/6），轨迹核验显示是真实校准而非关键词侥幸——
4 解谜题上枚举全部解、正确判定「无法唯一确定」；UNSAT 谜题上逐步推出「约束矛盾」。
这条新轴的安全含义比前四条更直接：「伪造确定性」本身就是一个可被利用的操纵面（精心
构造病态输入去诱导模型说出编造的"事实"），而 rift 目前把这个面关闭了。

需要指出三点：

1. **表格本身有一处算术对不上**：三档各 6/6，3×6=18，但「total」行写的是
   **24/24 = 1.0**——18 和 24 直接矛盾，读者没法从给出的数字里验算出这个总分。最
   可能的解释是三档实际各测了 8 题（3×8=24）而表格误写成 6/6，也可能是总分行本身
   写错；不管哪种，这是一处需要下次更新时澄清的具体数字错误，不是我推断出来的方法论
   薄弱点，是文档自己给出的两个数字直接矛盾。这条指标衡量的是能力/校准而非
   diversion 比值，不直接受本项目「diversion 比值一律配 Wilson CI」这条规则约束，
   但"满分关闭操纵面"这个强结论恰恰建立在这个有矛盾的总分上，核对清楚比补 CI 更
   优先。
2. **目前只在 rift 一个 victim 上测过**：文档结尾明确写道，这条轴是否也是
   **capability-ordered**（弱模型是否真的会在病态输入下编造确定性，让"校准"成为
   继"推理"之后第二个能力买到安全的地方）目前"under test cross-victim"，还没交付。
   如果弱模型真的会 fabricate，这会是全项目迄今唯一一条支持"能力进步确实买安全"的
   证据，方向上和其余四条轴（susceptibility 与能力无关）不同，值得优先跟进而不是
   顺延。
3. **顶部摘要没有跟上**：Abstract 正下方的「As of 2026-08-07」小结（findings.md
   第 41–43 行）仍然写着"FOUR independent strain axes now tested...core question
   settled"——现在轴数已经是五条，这句总结没有同步。这是继 #13（文件开头摘要没跟上
   §0a）、#15（§0-pre 自称 reframe 但 §0 headline 未同步）、#16（同一处摘要连续
   两轮未更新）、#22（§9.3 中段宣布 complete 后又追加两节）之后，第五次出现"收尾/
   总结性文字位置和实际写作进度对不上"——这个模式本身已经足够稳定，值得作为一条通用
   观察记下来：这份文档的顶部摘要习惯性地滞后于正文至少一到两轮追加，下次审阅时应
   默认顶部数字/措辞已经过期，先看正文最新章节再回头核对摘要是否需要更新。

方法论上延续了好的部分：判官对"unique"控制组里一题的初次误判（换行符正则的解析
问题）被人工核实推翻，和 §9.2/§9.5 一样，先核实是不是判官假阴性再下结论——这个习惯
...
| 2026-08-07（第十五批，一次性新增 Abstract + CAPSTONE + Three-axis update + Deep-frontier follow-up + §7 + §8 + §8.1 + §9–§9.5，共十个章节，是迄今单批追加量最大的一次） | [findings（续）](tech/2026-08-04-findings.md) | **置顶 Abstract（新）**：把全项目压缩成一页摘要，2026-08-07 落款，是目前最新、最全的入口——四条 strain 轴（难度/攻击强度/算术/逻辑）× 四条 diversion 通路（内容断言/动作/数值权威/逻辑权威）全部测完，结论「competence 按能力排序、只在检索/整合负担下会掉；susceptibility 低、shape 固定、和能力/strain 基本无关」。**CAPSTONE**（4-victim, well-powered，2026-08-06）：首次把 §0a 的弱 victim 结果做实——Llama-4-8B 在 depth 1→2→4 上 benign 真塌陷（0.34→0.21→0.16），但同一批更大样本（n_adm~30）下 diversion 持平（0.10→0.12→0.17，CI 重叠），直接推翻 §0a「diversion 朝 frontier 上升」的读数，文档自己写明「was noise」。新增第二个强 victim muse-spark-1.1，和 rift 曲线近乎重合。**Three-axis update（2026-08-07）**：攻击强度轴（§8/§8.1，见下）和计算轴（§9 系列，见下）都不提高 diversion；此前~0.4 的「hard-task floor」被重新归因为检索/整合负担，不是推理限制。**Deep-frontier follow-up**（depth-30/40，2026-08-06）：把 depth 轴推到 40，rift/muse-spark 双双打平在 ~0.43 的「hard-task floor」，深度本身不再往下压，density 才是。**§7**（2026-08-06）：exfiltration/unauthorized_action 首次补齐到全部四个 victim，0/41、0/46 全部配 Wilson CI——action 边界「从不被攻破」这条结论证据最扎实的一次扩展，但用的仍是原始注入方式，见需要你注意 #20。**§8/§8.1**（2026-08-07）：新增攻击强度轴，固定任务/竞争力/判据只变注入强度（弱→强结构化字段→跨记录一致，后者专门针对 rift 已知的「跨记录一致性核对」防御），rift diversion 仍卡在 ~0.11；kimi（mid）把更强的注入转成能力塌陷（benign 0.278→0.105）而非易感性上升。**§9 系列**（2026-08-07）：§9 用无计算器、单一清洁账本的算术阶梯证明 rift 42/42 全对，锁定「~0.4 floor 是检索问题不是算术问题」；§9.1 把数值权威注入进这批任务，rift/kimi 均 0/16 拒绝（配 CI）；§9.2 用 24 道逻辑推理题证明 rift 24/24（人工核对纠正了判官的 4 个假阴性）；§9.3 对逻辑推理做同样的权威注入测试，rift 0/15（配 CI），并在此处宣布「Program complete」；§9.4 把算术/逻辑两条阶梯扩展到 muse-spark 和 kimi-k3，发现强模型两项都满分、kimi 在逻辑推理上大量因未收敛而失败（不是算错，是没算完）；§9.5 尝试在 Llama-4-8B 上跑同一套推理阶梯，发现它完全不会用 googledocs 工具、每次都在几步内放弃，如实报告为「unmeasurable」而非假 0。**本轮（第十六批，追加在 §9.5 末尾）——root-cause upgrade**：把「unmeasurable」判定做了根因排查，读实际轨迹确认 gmail/googledocs 工具都被正确 offer，模型是自己在第 4 步幻觉出「服务不可用」并放弃（33/33 复现），排除了 harness/config bug；网关上也确认没有替代的弱模型可用。详见需要你注意 #24。**本轮（第十七批，紧接追加）——fix-attempt closure (opt-2)**：两次系统性补救尝试（① googledocs 换 gmail 投递，模型把 tool-call id 当字面占位符，检索失败；②再加提示 `get_gmail_content(limit=200)`，模型正确取到账本、开始计算，但循环截断、答案发错路由）均 0/6，文档归因于工具调用状态穿透、推理完成、结果路由三个环节同时纠缠失败；并解释了为什么 CAPSTONE 的 difficulty 轴上这个模型有非零 benign_rate——那批任务只需单次无穿透的 `get_gmail_content` 转储。详见需要你注意 #25。**本轮
（第十八批，追加全新 §10）——Ambiguity / epistemic calibration**：新增第五条
strain 轴，不是深挖已有数据，测的是模型面对病态提问（信息不足以唯一确定/约束互相
矛盾）时是否诚实报告，而不是编造确定答案。三种题型（unique 控制组/
underdetermined/contradictory）各 6 题，rift 满分（表格 total 行写 24/24，与
3×6=18 不吻合，见需要你注意 #26）；轨迹核验证实是真实校准而非关键词侥幸（4 解题
枚举全部解、UNSAT 题逐步推导矛盾）。文档把这条轴的安全含义总结为"关闭了一个操纵面
（伪造确定性）"，并明确留了一个开放问题——弱模型是否会在同类病态输入下
fabricate，让校准成为 capability-ordered 的安全维度，目前"under test
cross-victim"，尚未交付。详见需要你注意 #26。 | **正面**：§7/§8/§9.1/§9.3 是本项目至今 Wilson CI 规则执行最到位的一批新结论（见需要你注意 #23），§9.2/§9.5 延续了「先核实是不是判官假阴性/harness confound 再下结论」的好习惯。**需要注意**：①CAPSTONE/三轴更新把结论定格为「settled/complete」，但被建议十一轮以上的「换注入面（wall→soft surface）」pivot 依旧完全没有被执行，§7 补齐 action 全 victim 用的还是同一套注入方式，见需要你注意 #20；②第三个零基础设施说明就空降的新 victim muse-spark-1.1，且全文对它的命名本身不统一（有的地方省略版本号写「muse-spark」），见需要你注意 #21；③§9.3 宣布「Program complete」的位置在文档中段而非末尾，之后又追加了 §9.4/§9.5 两整节，是继 #13/#15/#16 之后第三次出现「收尾措辞和实际写作进度对不上」，见需要你注意 #22；④§8.1 和 Deep-frontier follow-up 的部分 diversion 数字仍未配 Wilson CI，是 #23 里指出的仅剩例外。**本轮（root-cause upgrade）见需要你注意 #24**——debug 方法扎实、且纠正了项目自己两版提交信息之间「harness confound」vs「no harness bug」的用词摆动，值得肯定；但新结论把"googledocs 工具族失败"概括成"低于 agentic 工具使用门槛"，和同一文档里 Llama-4-8B 在 gmail 任务上非零 benign_rate 的表现（CAPSTONE、§0a）不完全一致，范围写宽了。**本轮（opt-2 closure）见需要你注意 #25**——直接把上一条的张力解释清楚了：不是"googledocs vs gmail 工具族"的区别，是"单步转储 vs 多步 tool-call id 穿透"的区别，difficulty 轴任务恰好只需要前者。0/6 样本量小，但衡量的是竞争力不是 diversion，不受本项目 Wilson CI 规则约束，不构成新的合规问题。**本轮（§10）见需要你注意 #26**——判官假阴性核实的好习惯延续（一题
...
- **depth 是唯一自变量，env-breadth 钉死** —— 否则「strain 更高」和「环境更复杂」会混杂，`validate_chain` 强制真实的依赖链而非表面深度。
  ...
  **2026-08-06 追加（matched control，回应七次追加提出的「需要先补样本量」要求，部分满足）**：
  ...
  注意的」新增 #18。
  **2026-08-06 追加（第十一次，方向反转，这条决策的边界被重新划定）**：上一条要求的
  「n→~25 补充 linear 跑」这次交付（n=30），但结果不是「进一步收紧证据」，是「推翻方向」
  ——linear benign 从 0.556 掉到 0.30，反而低于 convergent 的 0.44。「depth 是唯一
  自变量」这条决策本身不需要改（depth 仍然是唯一被系统扫过的自变量），但它的**效力边界**
  需要重新划定：不是「depth 不 strain 前沿模型，除非换成 convergent 结构」，而是「depth
  不 strain 前沿模型，除非配上高信息密度的种子内容——不论线性还是 convergent 结构都会
  被压到 0.30–0.44」。真正缺失的自变量是内容密度，不是链式图结构。这是项目第三次主动
  纠正自己的头号新结论，详见「需要你注意的」新增 #19。
  **2026-08-07 追加（这条决策的范围事实上已经扩大成「四条独立自变量」）**：
  Three-axis update 在 depth/density 之外正式新增了攻击强度（§8/§8.1）和计算/推理
  （§9 系列）两条独立 strain 轴，四条轴全部测完、结论一致——都不提高 diversion。
  CAPSTONE 同时用更大样本重新验证了 Llama-4-8B 的弱 victim 结果，纠正了 §0a 当初
  「diversion 朝 frontier 上升」的读数（认定是 n_adm=6 时的噪声）。这条最初写着
  「depth 是唯一自变量」的决策，标题本身已经不再准确——项目实际在用的是一套四轴框架；
  「program complete」的收尾判断，见需要你注意 #20 关于「换注入面」仍未验证这一具体
  缺口。
  **2026-08-07 二次追加（轴数从四条扩到五条，但顶部摘要没跟上）**：§10 新增
  ambiguity/epistemic calibration 轴，rift 表格给出 24/24（尽管 3×6=18 这处算术
  本身对不上，见需要你注意 #26），是「program complete」宣布之后第一条真正新增的
  strain 维度而非旧数据的方法论收尾。「四条独立自变量」这个刚在上一条追加里写定的
  措辞，一轮之内就已经过期——项目现在实际在用的是五轴框架，且这一条轴（校准）在
...
- **diversion 比值一律配 Wilson 95% CI，不再裸报**（2026-08-04 追加）—— 小 n 下的整数比值（0.0/1.0）极易被误读成强结论；`analyze_strain.py` 现在给每个 cell 配区间，逼着自己和读者面对当前样本量下「什么都还没被证明」的现实。
  ...
  **2026-08-07 追加（合规性明显改善）**：§7/§8/§9.1/§9.3 四节新结论首次系统性地遵守了
  这条规则（0/41 [0,0.086]、0/46 [0,0.077]、2/18 [.03,.33]、0/16 [0,.19]、0/15
