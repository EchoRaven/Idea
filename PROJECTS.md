...
- **下一步**：①（最高优先级，具体可执行，已连续建议十五轮、项目自称"program complete"
  ...
  name),不用再提醒。
  ⑧（本轮新增，优先级仅次于①，具体可执行，成本较低，可直接复用已有基础设施）
  findings.md 新增的第五条 strain 轴——ambiguity/epistemic calibration（rift
  24/24）——文档自己留了一个开放问题："弱模型是否也会诚实报告病态输入，还是会
  fabricate"，标注"under test cross-victim"但尚未交付；这条测试可以直接复用
  已有的 constraint-puzzle 生成器和 §9.4 已经建好的 muse-spark/kimi-k3/
  Llama-4-8B 接入，成本明显低于设计一条全新轴，建议优先交付——如果弱模型确实会
  fabricate，这会是全项目迄今唯一一条支持"能力买安全"的证据，方向和其余四条轴
  相反，价值很高；同时 §10 表格本身有一处算术不自洽（三档 6/6 之和是 18，total
  却写 24/24，见项目索引「需要你注意的」#26），下次更新时应顺手澄清是题量本身
  （比如实际各 8 题）写错，还是总分行写错。
