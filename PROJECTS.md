...
- **下一步**：①（最高优先级，具体可执行，已连续建议十六轮、项目自称"program complete"
  ...
  name),不用再提醒。
  ⑧（已部分交付，但结论仍未坐实，见下方⑨）findings.md §10 留下的开放问题——"弱模型
  是否也会诚实报告病态输入，还是会 fabricate"——本轮用 kimi（mid）测了一次
  （§10.1），但结果被文档自己判定为 confounded：kimi 的推理经常被截断，正确率
  数字（1/6、3/6、5/6）主要反映"没答完"；唯一干净的信号是 fabrication detector
  报告两类病态题 0/6+0/6，但没有配 Wilson CI，且测的是 kimi（mid）而非最可能出现
  效应、也最需要先确认能否操作 harness 的 Llama-4-8B（weak）。§10 表格的算术
  不自洽（三档 6/6 之和是 18，total 却写 24/24，见项目索引「需要你注意的」#26）
  这次也仍未被澄清。
  ⑨（本轮新增，优先级仅次于①，具体可执行，成本较低）把 ⑧ 的开放问题真正做实：
  (a) 给 §10.1 的两个 0/6 fabrication 数字补 Wilson CI（n=6 时上界约 0.39，应
  和其余 diversion 类比值一样标注出来，不要因为指标名字不是"diversion"就绕过
  项目自己的规则）；(b) 提高 kimi 的输出 token 上限重跑一次，把"被截断"这个混淆
  变量实测排除，拿到一个干净的 mid-model 数字；(c) 在 Llama-4-8B 上试跑同一批
  ambiguity 语料前，先想清楚怎么把"模型答不出来"（harness/agentic 局限，如
  §9.5 已反复出现的模式）和"模型没有诚实报告病态"这两种失败模式分开，否则大概率
  只会拿到又一个"unmeasurable"而不是真正的 fabricate/not-fabricate 答案；
  (d) 顺手把 §10 表格 6/6×3=18 vs total 24/24 的算术矛盾问清楚，是题量写错
  （比如实际各 8 题）还是总分行写错。
...
新增 #25。**2026-08-07 十五次新增（本次处理的更新，findings.md 在 §9.5 之后新增
全新 §10，是"program complete"之后第一次真正扩展研究范围，不是方法论收尾）**：
新增第五条 strain 轴——ambiguity/epistemic calibration，测的是模型面对病态提问
（信息不足以唯一确定答案、或约束本身互相矛盾）时会不会编造一个自信的错误答案，
而不是如实报告"无法确定"/"矛盾"。三种题型（unique 控制组、underdetermined、
contradictory/UNSAT）各 6 题，rift 三档全部满分；轨迹核验证实是真实校准而非
关键词侥幸——4 解谜题上枚举全部解、正确判定"无法唯一确定"，UNSAT 谜题上逐步
推出"约束矛盾"。但表格给出的 total 行写着 24/24，和 6+6+6=18 直接对不上，是文档
自己的一处算术矛盾，不是我推断出来的方法论薄弱点。这条轴的安全含义比前四条更
直接——"伪造确定性"本身就是一个可被利用的操纵面，rift 目前把它关闭了——但文档
明确写道这条轴是否 capability-ordered（弱模型是否也会在病态输入下 fabricate，
让"校准"成为继"推理"之后第二个能力买到安全的地方）目前"under test
cross-victim"，尚未交付。这不改变任何一条既有卡点——四轴×四通路的核心结论不变，
"换注入面"pivot 依旧原样悬着——但如果弱模型确实会 fabricate，这会是全项目迄今
唯一一条支持"能力进步确实买安全"的证据，方向和其余四条轴相反，值得优先跟进（见
下一步⑧）。另外顶部 Abstract 正下方的"As of 2026-08-07"小结仍写着"FOUR
independent strain axes...core question settled"，没有同步到现在的五轴——是
"文档顶部摘要滞后于正文"这个模式第五次出现。详见项目索引「需要你注意的」新增 #26。
**2026-08-07 十六次新增（本次处理的更新，findings.md 在 §10 之后新增 §10.1，直接
回应上一轮下一步⑧的建议）**：kimi（mid）在同一批病态提问语料上测了一次，想验证
"弱模型是否会在信息不足/矛盾输入下 fabricate"这个开放问题——如果成立会是全项目
迄今唯一支持"能力买安全"的证据。结果被文档自己定性为 confounded：kimi 把推理写成
一整段思维链塞进最终消息，经常推理到一半被截断，按题型算的正确率（unique 1/6、
underdetermined 3/6、contradictory 5/6）主要反映"没答完"，不是"校准失败"。唯一
读得干净的信号：一个独立的 fabrication detector 在两类病态题（各 6 题）上全部报告
0/6——推理内容是老实的 case 枚举，不是自信编造。初步结论是 mid 模型大概率也不
fabricate（校准看起来"universal"），但这不是一个确定性答案，且这次测的是 kimi
（mid），不是最有希望看到"能力买安全"效应的 Llama-4-8B（weak）——Llama-4-8B 在
推理阶梯上已经证实有严重的 agentic harness 局限（§9.5，因工具调用状态穿透失败而
非推理能力本身导致 0/21、0/12），直接拿它测 ambiguity 语料很可能重演同一种
"unmeasurable"而非"fabricate/not"的结果。另外这两个 0/6 依旧没有配 Wilson CI
（n=6 时 0/6 的上界约 0.39），延续了项目自己"diversion/攻陷类比值一律配区间"规则
在新结论里又一次没有被执行的模式。这是一次值得肯定的响应速度——上一轮"下一步⑧"
建议的成本最低、复用现成基础设施的这条测试，一轮之内就被交付了，和"换注入面"pivot
连续十五轮未被采纳形成对比，说明这条 loop 不是不会响应具体建议，只是这次响应的不是
那条建议；"校准是否 capability-ordered"这个开放问题因此依旧未被真正回答。详见
项目索引「需要你注意的」新增 #27。
- **更新**：2026-08-07
