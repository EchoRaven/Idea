...
- **下一步**：①（高优先级,具体可执行,过去五轮已建议、本轮——findings §0a 的 Llama-4-8B
  ...
  ⑫（本轮新增,机会性,等外部结果)convergent 语料的 Llama-4-8B / kimi-k3 跨 victim 结果
  一旦跑完,直接检验这条新轴是否也复现 §0a 那种"能力越弱越先被压垮"的梯度,和 depth 轴的
  跨 victim 结果做横向对比——如果 convergent 轴在弱 victim 上同样让 diversion 明显上升
  (不同于它在 rift 上的 0.125,接近噪声地板),这会是比 depth 轴更强的一条支持核心机制
  的证据,值得单独成节。
- **卡点**：depth 轴、distractor-density 轴、discrimination 轴、以及 within-victim
  相关性检验(§2d)四条证据都收敛到同一个负结果,项目下一步依赖一个连续四轮建议、但连本轮
  (第六次,本轮是 §0a 的 Llama-4-8B 跨 victim 结果)仍未被执行的转向——从「继续在复杂度/
  相关性方向加证据」切到「换注入面(wall→soft surface)」;fork (b)(换一个真正弱的
  ...
  重复旧证据,两条路径(convergent 轴、注入面)现在都值得做,不是谁排挤谁。
- **更新**：2026-08-05
